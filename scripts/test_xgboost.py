#!/usr/bin/env python3
"""
Test script for the XGBoost Model Implementation.
Tests early stopping, feature importance analysis, hyperparameter tuning, cross-validation, and all functionality.
"""
...
from ml_models.tree_based.xgboost_model import (
    XGBoostModel, XGBoostConfig, ModelType,
    create_xgboost_classifier, create_xgboost_regressor, create_xgboost_with_early_stopping
)
import structlog

logger = structlog.get_logger()

class XGBoostTestSuite:
    """Comprehensive test suite for the XGBoost implementation."""
    ...
    async def test_basic_xgboost_classifier(self):
        """Test basic XGBoost classifier functionality."""
        logger.info("Testing basic XGBoost classifier")
        
        try:
            # Create model
            config = XGBoostConfig(n_estimators=50, max_depth=6, learning_rate=0.1)
            model = XGBoostModel(config, ModelType.CLASSIFIER, self.sample_data['feature_names'])
            ...
            logger.info("Basic XGBoost classifier test passed",
                       prediction_accuracy=np.mean(predictions == y))
            
        except Exception as e:
            self.test_results['basic_classifier'] = False
            logger.error("Basic XGBoost classifier test failed", error=str(e))
            raise
    
    async def test_basic_xgboost_regressor(self):
        """Test basic XGBoost regressor functionality."""
        logger.info("Testing basic XGBoost regressor")
        ...
        try:
            ...
            # Create model
            config = XGBoostConfig(n_estimators=50, max_depth=6, learning_rate=0.1)
            model = XGBoostModel(config, ModelType.REGRESSOR, self.sample_data['feature_names'])
            ...
            logger.info("Basic XGBoost regressor test passed",
                       mse=np.mean((predictions - y_reg) ** 2))
            
        except Exception as e:
            self.test_results['basic_regressor'] = False
            logger.error("Basic XGBoost regressor test failed", error=str(e))
            raise
    ...
    async def test_feature_importance_analysis(self):
        ...
        try:
            ...
            config = XGBoostConfig(n_estimators=100, max_depth=6, learning_rate=0.1)
            model = XGBoostModel(config, ModelType.CLASSIFIER, self.sample_data['feature_names'])
            ...
            importance_result = model.analyze_feature_importance(X, y, importance_type='weight', top_n=10)
            ...
            assert len(importance_result.importance_types) >= 0, "Should have importance types dict"
    ...
    async def test_hyperparameter_tuning(self):
        ...
        try:
            # Create model
            config = XGBoostConfig(n_estimators=50, max_depth=3, learning_rate=0.1)
            model = XGBoostModel(config, ModelType.CLASSIFIER, self.sample_data['feature_names'])
            ...
            # Test random search
            config2 = XGBoostConfig(n_estimators=30, max_depth=3, learning_rate=0.1)
            model2 = XGBoostModel(config2, ModelType.CLASSIFIER, self.sample_data['feature_names'])
    ...
    async def test_convenience_functions(self):
        ...
        try:
            ...
            assert classifier.config.max_depth == 6, "Should set max_depth"
            
            # Test regressor creation
            regressor = create_xgboost_regressor(
                n_estimators=75, learning_rate=0.15, max_depth=8, 
                feature_names=self.sample_data['feature_names']
            )
            ...
            assert regressor.config.max_depth == 8, "Should set max_depth"
            
            # Test early stopping model creation
            early_stop_model = create_xgboost_with_early_stopping(
    ...
    async def test_model_summary_and_visualization(self):
        ...
        try:
            ...
            config = XGBoostConfig(
                n_estimators=100, max_depth=6, learning_rate=0.1, early_stopping_rounds=10
            )
            model = XGBoostModel(config, ModelType.CLASSIFIER, self.sample_data['feature_names'])
            ...
            required_keys = ['model_type', 'is_fitted', 'n_estimators', 'learning_rate', 'max_depth', 'feature_count', 'best_iteration']
    ...
    async def test_edge_cases(self):
        ...
        try:
            ...
            model = XGBoostModel(config, ModelType.CLASSIFIER)
    ...
    async def run_all_tests(self):
        """Run all test cases."""
        logger.info("Starting XGBoost Test Suite")
        
        test_methods = [
            self.test_basic_xgboost_classifier,
            self.test_basic_xgboost_regressor,
        ...
        logger.info("XGBoost Test Suite completed",
...
async def main():
    ...
    test_suite = XGBoostTestSuite()
    success = await test_suite.run_all_tests()
    
    if success:
        logger.info("XGBoost Test Suite completed successfully")
        sys.exit(0)
    else:
        logger.error("XGBoost Test Suite failed")
        sys.exit(1)
